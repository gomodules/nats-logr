...
// Here conn is nats streaming server connection and subject is the publishing channel of nats server
func InfoDepth(depth int, conn stan.Conn, subject string, args ...interface{}) {
	logging.printDepth(infoLog, depth, conn, subject, args...)
}

// ErrorDepth acts as Error but uses depth to determine which call frame to log.
// ErrorDepth(0, "msg") is the same as Error("msg").
func ErrorDepth(depth int, conn stan.Conn, subject string, args ...interface{}) {
	logging.printDepth(errorLog, depth, conn, subject, args...)
}
